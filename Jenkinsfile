		...
		stage('Publish') {
			steps {
				withCredentials([[$class: 'StringBinding', credentialsId: 'Github', variable: 'TOKEN']]) {
					def filesToPush = findFiles(glob: '**.jar')
					releaseToGitHub('jeffmaury' , 'spring-boot-rest', '${TOKEN}', 'v0.0.1', '${filesToPush[0].path}')
				}
			}
		}
	}
...
def releaseToGitHub(owner, repo, token, tag, artifact) {
	...
	then
		...
		artifactId=\$(curl -XPOST -H "Authorization:token ${token}" --data-binary "@${artifact}" "https://api.github.com/repos/${owner}/${repo}/releases/\$releaseId/assets?name=\$name"| sed -n -e 's/"id":\\ \\([0-9]\\+\\),/\\1/p' | head -n 1 | sed 's/[[:blank:]]//g')
		if [ -z "\$artifactId" ]
		then
